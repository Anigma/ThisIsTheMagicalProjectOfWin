...
unsigned int probeUCStack(ucontext_t *uc);
unsigned int getEIPFromContext(ucontext_t *uc);
unsigned int getESPFromContext(ucontext_t *uc);
unsigned int getUESPFromContext(ucontext_t *uc);
...
int main(int argc, char **argv)
{
  ...
  printf("A ucontext_t is %d bytes\n", (int) sizeof(mycontext));
  assert(sizeof(mycontext) == 348); // TBD: Fill in ucontext size above. Hint: use sizeof().
  ...
  printf("The stack pointer stored as one of the registers (ESP) in uc_mcontext is 0x%.8x\n", getESPFromContext(&mycontext));
  printf("The stack pointer stored as another one of the `registers` (UESP) in uc_mcontext is 0x%.8x\n", getUESPFromContext(&mycontext));


  printf("The number of bytes pushed onto the stack between argc and err was 0x%.8x\n", (unsigned int) (&argc - &err) );
  /* Which is the right one to use? */
  printf("The number of bytes pushed onto the stack between err and when the stack was saved to mycontext was 0x%.8x\n", 
         (unsigned int) (&err - getESPFromContext(&mycontext)) );
 ...
 */
unsigned int 
probeUCStack(ucontext_t *uc)
{
  unsigned int ret = (unsigned int) ((uc->uc_stack).ss_sp);
  return ret;
}


unsigned int
getEIPFromContext(ucontext_t *uc)
{
  unsigned int ret = (unsigned int) (uc->uc_mcontext).gregs[REG_EIP];
  return ret;
}

unsigned int
getESPFromContext(ucontext_t *uc)
{
  unsigned int ret = (unsigned int) (uc->uc_mcontext).gregs[REG_ESP];
  return ret;
}

unsigned int
getUESPFromContext(ucontext_t *uc)
{
  unsigned int ret = (unsigned int) (uc->uc_mcontext).gregs[REG_UESP];
  return ret;
}
